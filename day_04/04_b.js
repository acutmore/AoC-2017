...
function valid(line) {
    ...
    const map = {};
    words
        .map(word => anagramHash(word))
        .forEach(v => map[v] = void 0);
    return words.length === Object.keys(map).length;
}

function anagramHash(word) {
    ...
    const hash = letters.map(letter => `|${letter}-${buckets[letter]}|`)
        .reduce((a, b) => a + b, '');
    return hash;
}